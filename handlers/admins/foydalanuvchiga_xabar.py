...
async def sendmessage(msg: types.Message, state: FSMContext):
    if msg.text.isnumeric():
        ...
        await msg.answer(
            text="<b>Reklama yuborish : </b>\n<i>(Oddiy xabar, Rasm, Video, Dokument)</i>",
...
async def func(msg: types.Message, state: FSMContext):
    try:
        await bot.forward_message(
            temp_data[msg.from_user.id], msg.from_user.id, msg.message_id
        )
    except:
        pass

    await msg.answer(
        f"<b>Foydalanuvchiga xabar yuborildi ✅</b>", reply_markup=admin_key
    )
    await state.finish()
