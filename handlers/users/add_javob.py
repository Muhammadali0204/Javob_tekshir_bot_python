...
@dp.message_handler(
    filters.ChatTypeFilter(types.ChatType.PRIVATE), text="🟢Testga javob berish"
)
    ...
    if kanallar[0] != "":
        ...
        if not final_status:
            ...
            await msg.answer(
                ...
                reply_markup=channels_keyboard(azo_bulmagan_kanallar, "javob_ber"),
            )
        else:
            ...
            if user == None:
                ...
                return
            await msg.answer(
                "<b>Yaxshi, test kodini yuboring : </b>", reply_markup=bekor_qil
            )
    ...
    if kod.isnumeric():
        ...
        if data_test_blok == None and data_test_oddiy == None:
            ...
            if test_kodi < hozirgi_kod:
                await msg.answer(
                    "<b>Bu test allaqachon yakunlangan!</b>\n", reply_markup=menu
                )
            else:
                await msg.answer(
                    text="<b>Bunday kodli test mavjud emas!</b>", reply_markup=menu
                )
            await state.finish()
        else:
            if data_test_blok == None:
                if data_test_oddiy[5] == 1:
                    ...
                    if javob_bergan == None:
                        ...
                        answer += (
                            f"🔢<b>Savollar soni : {len(data_test_oddiy[3])} ta</b>\n\n"
                        )
                        ...
                        await msg.answer(
                            f"<b>Siz {kod} - kodli testga javob berib bo`lgansiz❗️</b>",
                            reply_markup=menu,
                        )
                        await state.finish()
                else:
                ...
                if data_test_blok[6] == 1:
                    ...
                    if javob_bergan == None:
                        ...
                        await msg.answer(
                            f"<b>Siz {kod} - kodli blok testga javob berib bo`lgansiz❗️</b>",
                            reply_markup=menu,
                        )
                        await state.finish()
                else:
        ...
        await msg.answer(
            "<b>Test kodi natural son!</b>\n<i>Qayta kiriting yoki bekor qiling❗️</i>",
