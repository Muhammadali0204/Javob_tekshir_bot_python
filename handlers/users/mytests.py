...
@dp.message_handler(
    filters.ChatTypeFilter(types.ChatType.PRIVATE), text="📋Mening testlarim"
)
async def mytest(msg: types.Message, state: FSMContext):
    ...
    await msg.answer(
        "<b>Test turini tanlang 👇</b>", reply_markup=oddiy_blok_test.oddiy_blok
    )
    await state.set_state("my_tests")


@dp.message_handler(
    state=["my_tests", "my_tests_blok", "my_tests_oddiy"],
...
async def my_tests_oddiy(call: types.CallbackQuery, state: FSMContext):
    ...
    if tests_oddiy == []:
        ...
        await call.message.answer(
            "<b>📕Oddiy testlar : </b>",
            reply_markup=test_list.test_listt_oddiy(tests_oddiy),
        )
...
async def my_tests_blok(call: types.CallbackQuery, state: FSMContext):
    ...
    if tests_blok == []:
        ...
        await call.message.answer(
            "<b>📚Blok testlar : </b>",
            reply_markup=test_list.test_listt_blok(tests_blok),
        )
...
async def ortgaa(call: types.CallbackQuery, state: FSMContext):
    await call.message.delete()
    await call.message.answer(
        "<b>Tanlang 👇</b>", reply_markup=oddiy_blok_test.oddiy_blok
    )
    await state.set_state("my_tests")
