...
@dp.callback_query_handler(
    filters.ChatTypeFilter(types.ChatType.PRIVATE), regexp="tugatish:+"
)
async def tugatish(call: types.CallbackQuery):
    ...
    if data_test_blok == None and data_test_oddiy == None:
        await call.answer(
            f"{test_kodi} - kodli test allaqachon yakunlangan❗️", show_alert=True
        )
        await call.message.delete()
    else:
        if data_test_blok == None:  # Test oddiy bo`lsa
            javob_berganlar_malumoti = (
                db_bj.select_all_javob_berganlar_tartiblangan_oddiy_by_testkodi(
                    test_kodi
                )
            )
        ...
        #                                      id            ismi    username  sta  kanal     user_id       kodi    javoblar  xato javoblar
            ...
            javob_berganlar_malumoti = (
                db_bj.select_all_javob_berganlar_tartiblangan_blok_by_testkodi(
                    test_kodi
                )
            )
            javob_berganlar = []  # [(1, 2, 3), (4, 5, 6, ) ... ]
