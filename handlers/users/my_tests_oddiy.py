...
@dp.callback_query_handler(regexp="kanal_guruh_joylash_yoqish:+", state="oddiy_testni_tahrirlash")
...
@dp.callback_query_handler(regexp="kanal_guruh_joylash_ochirish:+", state="oddiy_testni_tahrirlash")
async def kkncsknskd(call : types.CallbackQuery, state  :FSMContext):
    test_kodi = call.data.split(':')[1]
    user = db_users.select_user_id(call.from_user.id)
