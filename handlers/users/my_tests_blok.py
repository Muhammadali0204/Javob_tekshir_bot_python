...
async def ortgaaa(call: types.CallbackQuery, state: FSMContext):
    ...
    await call.message.answer(
        "<b>📚Blok testlar : </b>", reply_markup=test_list.test_listt_blok(tests_blok)
    )
    await state.set_state("my_tests_blok")


@dp.callback_query_handler(
    state="blok_testni_tahrirlash", regexp="javob_berganlar_soni:+"
)
...
@dp.callback_query_handler(
    state="blok_testni_tahrirlash", regexp="fan_nomini_tahrirlash:+"
)
async def qatnashganlar_soni(call: types.CallbackQuery, state: FSMContext):
    ...
    await call.message.answer(
        "<b>Tahrirlamoqchi bo'lgan faningizni tanlang 👇</b>",
        reply_markup=blok_test_fan_tah.blok_test_fanlar(fanlar, test_kodi),
    )
...
async def tahrirlash(msg: types.Message, state: FSMContext):
    if all(x.isalpha() or x.isspace() or isbelgi.isbelgi(x) for x in msg.text):
        if len(msg.text) > 2 and len(msg.text) < 31:
            ...
            test = db_ts.select_test_blok_by_test_kodi(temp_data[msg.from_user.id][0])
            fan_nomlari = test[2].split(",")
            fan_nomlari[int(temp_data[msg.from_user.id][1])] = fan_nomi
            fan_nomi1 = ",".join(fan_nomlari)
            db_ts.update_fan_nomi_blok_test(temp_data[msg.from_user.id][0], fan_nomi1)
            await msg.answer(
                f"<b>{temp_data[msg.from_user.id][0]} - kodli blok testning {int(temp_data[msg.from_user.id][1]) + 1} - fani nomi {fan_nomi} ga o'zgardi ♻️</b>",
...
@dp.callback_query_handler(
    regexp="kanal_guruh_joylash_yoqish:+", state="blok_testni_tahrirlash"
)
...
@dp.callback_query_handler(
    regexp="kanal_guruh_joylash_ochirish:+", state="blok_testni_tahrirlash"
)
async def kkncsknskd(call: types.CallbackQuery, state: FSMContext):
    test_kodi = call.data.split(":")[1]
    user = db_users.select_user_id(call.from_user.id)
...
async def djcnk(call: types.CallbackQuery, state: FSMContext):
    ...
    javob_berganlar_malumoti = (
        db_bj.select_all_javob_berganlar_tartiblangan_blok_by_testkodi(test_kodi)
    )
