...
@dp.callback_query_handler(
    text="javob_yuborishni_boshla", state="javob_yuborishni_boshlash_blok"
)
...
@dp.callback_query_handler(
    text="javob_yuborishni_bekor_qil", state="javob_yuborishni_boshlash_blok"
)
...
async def yes(call: types.CallbackQuery, state: FSMContext):
    ...
    if temp_data[call.from_user.id][4] != temp_data[call.from_user.id][0]:
        ...
        await call.answer(
            f"{temp_data[call.from_user.id][4]} - fanning javoblari qabul qilindi✅"
        )
        await call.message.delete()
        await call.message.answer(text=answer)
        await state.set_state("javob_yuborish_blok")
        ...
        if data_test == None:
            ...
            tuplagan_ballari = ",".join([f"{item}" for item in tuplagan_ballari_temp])
            xato_javoblari = ",".join([f"{item}" for item in temp_xato_javoblari])
            summ = sum(tuplagan_ballari_temp)
            try:
                db_bj.add_javob_blok(
                    user_id, test_kodi, tuplagan_ballari, xato_javoblari, summ
                )
            ...
            await call.answer(
                f"{test_kodi} - testga javob berdingiz ✅", show_alert=True
            )
            await call.message.delete()
            await state.finish()
            ...
            await bot.send_message(
                data_test[0], answer_admin, reply_markup=test_owner(test_kodi)
            )
