...
class Database:
    ...
    def add_user(self, user_id, name: str, username: str, status, kanal):  # this
        ...
        """
        self.execute(
            sql, parameters=(user_id, name, username, status, kanal), commit=True
        )
    ...
    def kanallar(self):
        return self.execute(
            sql="SELECT username FROM Users WHERE user_id = 2", fetchone=True
        )

    def count_users(self):  # this
        return self.execute(
            "SELECT COUNT(*) FROM Users WHERE user_id > 100", fetchone=True
        )
    ...
    def take_test_kodi(self):  # this
        ...
        self.execute(
            "UPDATE Users SET username = ? WHERE user_id = 1",
            parameters=(str(new_code),),
            commit=True,
        )
        return data[0]
    ...
    def update_premium_narx(self, narx):
        self.execute(
            sql="UPDATE Users SET username = ? WHERE user_id = 5",
            parameters=(narx,),
            commit=True,
        )
    ...
    def update_kanal_user(self, kanal, id):
        self.execute(
            sql="UPDATE Users SET kanal = ? WHERE user_id = ?",
            parameters=(kanal, id),
            commit=True,
        )
    ...
    def add_test_oddiy(
        self, user_id, test_kodi, fan_nomi, javoblar: str, avto_vaqt, faollik, avto_post
    ):  # this
        sql = """
    ...
    def all_tests_oddiy_sana(self, sana):
        return self.execute(
            sql="SELECT * FROM Oddiy_test WHERE avto_vaqt = ? ",
            parameters=(sana,),
            fetchall=True,
        )
    ...
    def all_tests_blok_sana(self, sana):
        return self.execute(
            sql="SELECT * FROM Blok_test WHERE avto_vaqt = ? ",
            parameters=(sana,),
            fetchall=True,
        )
    ...
    def all_tests_oddiy_vaqt(self):
        return self.execute(
            sql="SELECT * FROM Oddiy_test WHERE avto_vaqt LIKE '%,%' ", fetchall=True
        )

    def all_tests_blok_vaqt(self):
        return self.execute(
            sql="SELECT * FROM Blok_test WHERE avto_vaqt LIKE '%,%' ", fetchall=True
        )

    def select_tests_by_user_id_oddiy(self, id):
        return self.execute(
            sql="SELECT * FROM Oddiy_test WHERE user_id = ?",
            parameters=(id,),
            fetchall=True,
        )
    ...
    def select_tests_by_user_id_blok(self, id):
        return self.execute(
            sql="SELECT * FROM Blok_test WHERE user_id = ?",
            parameters=(id,),
            fetchall=True,
        )
    ...
    def update_fan_nomi_oddiy_test(self, test_kodi, yangi_nom):
        self.execute(
            "UPDATE Oddiy_test SET fan_nomi = ? WHERE test_kodi = ?",
            parameters=(yangi_nom, test_kodi),
            commit=True,
        )

    def update_fan_nomi_blok_test(self, test_kodi, yangi_nom):
        self.execute(
            "UPDATE Blok_test SET fan_nomi = ? WHERE test_kodi = ?",
            parameters=(yangi_nom, test_kodi),
            commit=True,
        )
    ...
    def add_javob_oddiy(self, id, kod, tuplagan_bal, xato_javoblari):
        ...
        self.execute(
            sql, parameters=(id, kod, tuplagan_bal, xato_javoblari), commit=True
        )
    ...
    def add_javob_blok(self, id, kod, tuplagan_bal, xato_javoblari, jami):
        ...
        self.execute(
            sql, parameters=(id, kod, tuplagan_bal, xato_javoblari, jami), commit=True
        )

    def select_all_javob_berganlar_by_test_kodi(self, test_kodi):
        sql = "SELECT user_id FROM Oddiy_test WHERE test_kodi = ?"
        oddiy_testga_javoblar = self.execute(
            sql=sql, parameters=(test_kodi,), fetchall=True
        )
        if oddiy_testga_javoblar == []:
            sql = "SELECT * FROM Blok_test WHERE test_kodi = ?"
            blok_testga_javoblar = self.execute(
                sql=sql, parameters=(test_kodi,), fetchall=True
            )
    ...
    def count_answers_oddiy_test(self, test_kodi):
        return self.execute(
            "SELECT COUNT(user_id) FROM Oddiy_test WHERE test_kodi = ?",
            parameters=(test_kodi,),
            fetchone=True,
        )

    def count_answers_blok_test(self, test_kodi):
        return self.execute(
            "SELECT COUNT(user_id) FROM Blok_test WHERE test_kodi = ?",
            parameters=(test_kodi,),
            fetchone=True,
        )
